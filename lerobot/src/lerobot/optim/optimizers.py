...
from lerobot.optim.muon import MuonWithAuxAdam, SingleDeviceMuonWithAuxAdam
...
class MuonConfig(OptimizerConfig):
    """Muon optimizer config. Params with ndim >= 2 use Muon; others use Adam."""
    ...
    def build(self, params) -> torch.optim.Optimizer:
        flat_params = [p for p in params if p.requires_grad]
        muon_params = [p for p in flat_params if p.ndim >= 2]
        adam_params = [p for p in flat_params if p.ndim < 2]
        ...
        # Data-parallel sync handles cross-GPU communication; use single-device variant.
...
def _save_single_optimizer_state(optimizer: torch.optim.Optimizer, save_dir: Path) -> None:
    ...
    if "param_groups" in state:
        ...
        # Accelerate-wrapped optimizer may have different state_dict format; skip saving
        print(f"[save_optimizer] Warning: no 'param_groups' in state_dict (keys={list(state.keys())}), skipping optimizer state save.")
