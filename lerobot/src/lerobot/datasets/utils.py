...
def load_nested_dataset(pq_dir: Path, features: datasets.Features | None = None) -> Dataset:
    ...
    dsets = [Dataset.from_parquet(str(path), features=features) for path in paths]
...
def load_info(local_dir: Path) -> dict:
    ...
    for ft in info["features"].values():
        # Scalar features (int64 task_index, episode_index, ...) may omit the
        # shape key in info.json; treat that as a 0-d shape.
        ft["shape"] = tuple(ft.get("shape", ()))
    return info
...
def dataset_to_policy_features(features: dict[str, dict]) -> dict[str, PolicyFeature]:
    ...
    for key, ft in features.items():
        ...
        if ft["dtype"] in ["image", "video"]:
            ...
            # Default to (height, width, channels) axis order (LeRobot v3.0
            # standard) when `names` is missing from info.json.
            names = ft.get("names") or ["height", "width", "channels"]
