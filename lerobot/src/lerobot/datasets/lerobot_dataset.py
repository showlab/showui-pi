...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # When `delta_timestamps` are enabled, __getitem__ uses episode start/end indices to
        # clamp query indices (for action chunks, etc.). Some datasets may have stale/incorrect
        # `meta/episodes.dataset_{from,to}_index` that no longer matches the row order produced
        # by concatenating parquet shards. We therefore compute per-episode row ranges from the
        # loaded hf_dataset and prefer them for delta sampling when available.
        self._hf_episode_from_index: np.ndarray | None = None  # (total_episodes,)
        self._hf_episode_to_index: np.ndarray | None = None  # (total_episodes,)
        ...
        try:
            ...
            # Local-first fallback: if a local dataset directory exists (data/*.parquet present),
            # avoid any Hub access and reload directly from disk.
            try:
                from pathlib import Path as _Path
                data_dir = _Path(self.root) / "data"
                has_local = any(data_dir.glob("*/*.parquet"))
            except Exception:
                has_local = False

            if has_local:
                # Ignore cached-episodes check and proceed with local data
                self.hf_dataset = self.load_hf_dataset()
            else:
                # Fallback to Hub (original behavior)
                self.revision = get_safe_version(self.repo_id, self.revision)
        ...
        if self.delta_timestamps is not None:
            ...
            self._build_hf_episode_row_ranges()

    def _build_hf_episode_row_ranges(self) -> None:
        """Build episode→(row_start,row_end) from the loaded hf_dataset row order.

        This is used for delta sampling (`delta_timestamps`) so we don't rely on potentially
        stale `meta/episodes.dataset_{from,to}_index`.
        """
        if self.hf_dataset is None or len(self.hf_dataset) == 0:
            return

        try:
            # Avoid triggering set_transform (image decoding) by reading from Arrow directly.
            ep_col = self.hf_dataset.data.column("episode_index").to_pylist()
        except Exception as e:  # noqa: BLE001
            logging.warning(f"Failed to read episode_index column from hf_dataset: {e}.")
            return

        total_eps = int(self.meta.total_episodes)
        starts = np.full(total_eps, -1, dtype=np.int64)
        ends = np.full(total_eps, -1, dtype=np.int64)

        for row_idx, ep_idx in enumerate(ep_col):
            ep_i = int(ep_idx)
            if starts[ep_i] == -1:
                starts[ep_i] = row_idx
            ends[ep_i] = row_idx + 1  # exclusive

        # Validate contiguity: episodes should be stored as contiguous blocks in row order.
        counts = np.bincount(np.asarray(ep_col, dtype=np.int64), minlength=total_eps)
        has = counts > 0
        spans = ends[has] - starts[has]
        if not np.all(spans == counts[has]):
            logging.warning(
                "hf_dataset episode_index is not stored contiguously in row order; "
                "falling back to metadata dataset_from_index/to_index for delta sampling."
            )
            return

        self._hf_episode_from_index = starts
        self._hf_episode_to_index = ends

        # Log mismatch rate vs metadata once (helps catch stale episode metadata bugs).
        try:
            meta_st = np.asarray(self.meta.episodes["dataset_from_index"], dtype=np.int64)
            meta_en = np.asarray(self.meta.episodes["dataset_to_index"], dtype=np.int64)
            mismatch = has & ((meta_st != starts) | (meta_en != ends))
            if mismatch.any():
                n_has = int(has.sum())
                n_mis = int(mismatch.sum())
                logging.warning(
                    f"Detected {n_mis}/{n_has} episodes where metadata dataset_from_index/to_index "
                    "does not match hf_dataset row order. Using hf_dataset-derived ranges for delta sampling."
                )
        except Exception:
            pass

    def _get_episode_row_range(self, ep_idx: int) -> tuple[int, int]:
        """Return (row_start,row_end) for an episode in the current hf_dataset row order."""
        if self._hf_episode_from_index is not None and self._hf_episode_to_index is not None:
            st = int(self._hf_episode_from_index[ep_idx])
            en = int(self._hf_episode_to_index[ep_idx])
            if st >= 0 and en >= 0:
                return st, en
    ...
    def _check_cached_episodes_sufficient(self) -> bool:
        ...
        # Read episode indices directly from the underlying Arrow table to
        # avoid triggering set_transform (which would decode all images).
        available_episodes = set(self.hf_dataset.data.column("episode_index").to_pylist())
    ...
    def _get_query_indices(self, idx: int, ep_idx: int) -> tuple[dict[str, list[int | bool]]]:
        ep_start, ep_end = self._get_episode_row_range(ep_idx)
        query_indices = {
